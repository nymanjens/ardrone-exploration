...
namespace PTAMM {
    ...
    mvpMaps(maps), mpMap(m), mbResetRequested(false), mbResetDone(true), mbBundleAbortRequested(false), mbBundleRunning(false),
    mbBundleRunningIsRecent(false), mbReInitRequested(false), mbReInitDone(false), mbSwitchRequested(false),
    mbSwitchDone(false) {
        mkSrc = NULL;
        mkTarget = NULL;
        mkTargetBest = NULL;
...
#define CKECK_ABORTS CHECK_RESET CHECK_REINIT CHECK_SWITCH CHECK_UNLOCK
    ...
    bool MapMaker::InitFromStereo(KeyFrame &kF, KeyFrame &kS, vector<pair<ImageRef, ImageRef> > &vTrailMatches, SE3<> &se3TrackerPose) {
        ...
        if(!FAST_MAP_EXPANSION2) {
            ...
            mkSrc = pkSecond;
            mkTarget = pkFirst;
            AddSomeMapPoints(0, *pkSecond, *pkFirst); // .. and add more map points by epipolar search.
            AddSomeMapPoints(3, *pkSecond, *pkFirst);
            AddSomeMapPoints(1, *pkSecond, *pkFirst);
            AddSomeMapPoints(2, *pkSecond, *pkFirst);
        }
        /** </custom> ***/
        ...
        /** <custom> ***/
        // get scale to convert SLAM scale to meters
    ...
    void MapMaker::ApplyGlobalTransformationToMap(SE3<> se3NewFromOld) {
        ...
        for (unsigned int i = 0; i < mpMap->vpPoints.size(); i++) {
            mpMap->vpPoints[i]->v3WorldPos = se3NewFromOld * mpMap->vpPoints[i]->v3WorldPos;
    ...
    void MapMaker::AddKeyFrameFromTopOfQueue() {
        ...
        if(!FAST_MAP_EXPANSION2){
            ...
            mkSrc = (mpMap->vpKeyFrames[mpMap->vpKeyFrames.size() - 1]); // The new keyframe
            /*
             * Find a keyframe that will be the best match. Requirements are:
             * 1. New and old keyframes need to be looking at approximately
             * same spot in the world
             * 2.There needs to be a stereo separation between them, optimally
             * base to height ratio (B/H) should be around 1
             */
            mkTargetBest = NewBestMatchKeyFrame(*mkSrc);
            // Old method
            int nClosest;
            mkTarget = ClosestKeyFrame(*mkSrc, nClosest);
            //AddSomeMapPoints(3, *mkSrc, *mkTarget); // .. and add more map points by epipolar search.
            //AddSomeMapPoints(0, *mkSrc, *mkTarget);
            //AddSomeMapPoints(1, *mkSrc, *mkTarget);
            //AddSomeMapPoints(2, *mkSrc, *mkTarget);
            AddSomeMapPoints(3, *mkSrc, *mkTargetBest); // .. and add more map points by epipolar search.
            AddSomeMapPoints(0, *mkSrc, *mkTargetBest);
            AddSomeMapPoints(1, *mkSrc, *mkTargetBest);
            AddSomeMapPoints(2, *mkSrc, *mkTargetBest);
        }
        /** </custom> ***/
    ...
    bool MapMaker::AddPointEpipolar(KeyFrame &kSrc, KeyFrame &kTarget, int nLevel, int nCandidate) {
        ...
        Vector < 3 > v3New;
        v3New = kTarget.se3CfromW.inverse() * ReprojectPoint(kSrc.se3CfromW * kTarget.se3CfromW.inverse(),
                kSrc.Camera.UnProject(v2RootPos), kTarget.Camera.UnProject(Finder.GetSubPixPos()));
    ...
    void MapMaker::BundleAdjustRecent() {
        if (mpMap->vpKeyFrames.size() < 8) { // Ignore this unless map is big enough
        ...
        // (custom) comment
        //cout << getTime() << ": Recent BA run" << endl;
    }
    ...
    void MapMaker::BundleAdjust(set<KeyFrame*> sAdjustSet, set<KeyFrame*> sFixedSet, set<MapPoint*> sMapPoints, bool bRecent) {
        ...
        for (unsigned int i = 0; i < mpMap->vpKeyFrames.size(); i++) {
            ...
            for (meas_it it = mpMap->vpKeyFrames[i]->mMeasurements.begin(); it != mpMap->vpKeyFrames[i]->mMeasurements.end(); it++) {
                if (mPoint_BundleID.count(it->first) == 0)
                    continue;
                int nPoint_BundleID = mPoint_BundleID[it->first];

                b.AddMeas(nKF_BundleID, nPoint_BundleID, it->second.v2ImplanePos, LevelScale(it->second.nLevel) * LevelScale(
                        it->second.nLevel), it->second.m2CamDerivs);
            }
        }
        ...
        if (nAccepted < 0) {
            ...
            cout << "!! MapMaker: Cholesky failure in bundle adjust. " << endl << "   The map is probably corrupt: Ditching the map. "
                    << endl;
            mbResetRequested = true;
            return;
        }
    ...
    SE3<> MapMaker::CalcPlaneAligner() {
        ...
        for (int i = 0; i < nRansacs; i++) {
            ...
            Vector < 3 > v3Mean = 0.33333333 * (mpMap->vpPoints[nA]->v3WorldPos + mpMap->vpPoints[nB]->v3WorldPos
     ...
     * <custom><ptamv>
     */
    bool MapMaker::FindPointEpipolar(KeyFrame &kSrc, KeyFrame &kTarget, int nLevel, int nCandidate) {
        int nLevelScale = LevelScale(nLevel);
        Candidate &candidate = kSrc.aLevels[nLevel].vCandidates[nCandidate];
        ImageRef irLevelPos = candidate.irLevelPos;
        Vector < 2 > v2RootPos = LevelZeroPos(irLevelPos, nLevel);
     ...
     * <custom><ptamv>
     */
    int MapMaker::FindNrOfMatches(int nLevel, KeyFrame &kSrc, KeyFrame &kTarget) {
        // TODO: This operates on kSrc frame, also this should be called only once
        ThinCandidates(kSrc, nLevel);
        //cout << "After 'thinning', " << kSrc.aLevels[nLevel].vCandidates.size() << " canditate features are left." << endl;

        Level &l = kSrc.aLevels[nLevel];
        int nrOfMatches = 0;
        for (unsigned int i = 0; i < l.vCandidates.size(); i++)
            if (FindPointEpipolar(kSrc, kTarget, nLevel, i))
                nrOfMatches++;
     ...
     * <custom><ptamv>
     */
    KeyFrame* MapMaker::NewBestMatchKeyFrame(KeyFrame &kSrc) {
        double dDist = 0;
        double dClosestDist = DBL_MAX;
        int maxNrOfMatches = 0;
        int nBest = -1;

        // TODO: Shouldn't it use old keyframe scene depth estimate?
        double minStereoSeparation = GV2.GetDouble("MapMaker.MaxKFDistWiggleMult", 1.0, SILENT) * mdWiggleScaleDepthNormalized
                * kSrc.dSceneDepthMean;
        //        cout << "\n\n";
        //        cout << "New KF SrcDepth = " << kSrc.dSceneDepthMean << " , SrcSigma = " << kSrc.dSceneDepthSigma << endl;
        for (unsigned int i = 0; i < mpMap->vpKeyFrames.size(); i++) {
            KeyFrame& kTarget = *mpMap->vpKeyFrames[i];

            //            cout << "Trying to match with KF: " << i << endl;
            // Exclude comparing to the same keyframe
            if (mpMap->vpKeyFrames[i] == &kSrc) {
                //                cout << "Skipping identity frame" << endl;
                continue;
            }
            // Exclude keyframe if there is not enough stereo separation e.g. when a keyframe
            // was added due to change in viewing direction and not change in position
            dDist = KeyFrameLinearDist(kSrc, kTarget);
            if (dDist < minStereoSeparation) {
                //                cout << "Skipping since no enough separation: " << dDist << endl;
                continue;
            }

            // Higer the level, smaller the image and faster the search, also less acurate
            int nrOfMatches = 0;
            nrOfMatches += FindNrOfMatches(3, kSrc, kTarget);
            //            nrOfMatches += FindNrOfMatches(2, kSrc, kTarget);
            //            nrOfMatches += FindNrOfMatches(1, kSrc, kTarget);
            //            nrOfMatches += FindNrOfMatches(0, kSrc, kTarget);

            //            cout << "Found: " << nrOfMatches << " matches" << endl;

            if (maxNrOfMatches < nrOfMatches) {
                nBest = i;
                maxNrOfMatches = nrOfMatches;
            }

        }

        // Finding a match for a new keyframe failed, warn the user and try using distance only
        if (nBest == -1) {
            // Warn user
            //            cout << "NewBestMatchKeyFrame failed, defaulting to distance metric" << endl;

            dClosestDist = DBL_MAX;
            nBest = -1;
            for (unsigned int i = 0; i < mpMap->vpKeyFrames.size(); i++) {
                if (mpMap->vpKeyFrames[i] == &kSrc)
                    continue;
                dDist = KeyFrameLinearDist(kSrc, *mpMap->vpKeyFrames[i]);

                /*
                 * Try to pick a keyframe that has stereo separation and is close
                 * to the new keyframe i.e. try to pick the keyframe with distance
                 * closest to minStereoSeparation. This will penalize keyframes
                 * too close to the new keyframe as well as those that are far away.
                 * TODO:
                 */
                //if (dDist < dClosestDist) {
                if (abs(dDist - minStereoSeparation) < dClosestDist) {
                    dClosestDist = dDist;
                    nBest = i;
                }
            }
            ...
            if (dDist < minStereoSeparation) {
                // Warn user that the picked keyframe may not have enough stereo separation
                cout << "Not enough stereo-separation" << endl;
            }
        }

        //        cout << "\nNew algorithm picked KF: " << nBest << endl;
        assert(nBest != -1);
        return mpMap->vpKeyFrames[nBest];
    }
    
    /**
     * <custom><ptamv>
     */
    KeyFrame* MapMaker::ClosestKeyFrame(KeyFrame &k, int &nClosest) {
        //double dClosestDist = 9999999999.9;
        double dClosestDist = DBL_MAX;
        nClosest = -1;
        for (unsigned int i = 0; i < mpMap->vpKeyFrames.size(); i++) {
     ...
     * the ClosestKeyFrame function.
     */
    void MapMaker::AddSomeMapPoints(int nLevel, KeyFrame &kSrc, KeyFrame &kTarget) {
        ThinCandidates(kSrc, nLevel);
     ...
     * <custom><ptamv>
     */
    vector<pair<double, KeyFrame*> > MapMaker::ClosestKeyFrames(KeyFrame &k, double maxDistance) {
        vector<pair<double, KeyFrame*> > closeKFs;
        for (unsigned int i = 0; i < mpMap->vpKeyFrames.size(); i++) {
            if (mpMap->vpKeyFrames[i] == &k)
                continue;
            double dDist = KeyFrameLinearDist(k, *mpMap->vpKeyFrames[i]);
            if (dDist < maxDistance)
                closeKFs.push_back(make_pair(dDist, mpMap->vpKeyFrames[i]));
        }
        return closeKFs;
    }
    ...
    /*
     * <custom><ptamv>
     * Decides if we need to add a new keyframe or not. It is a heuristic using
     * 1. Either the camera is far enough from any keyframe already in the set
     * 2. Or the camera is panned into new direction.
     */
    bool MapMaker::NeedNewKeyFrameExtMode(KeyFrame &kCurrent, double &currentDistance, double &requiredDistance, double &currentAngle,
            double &requiredAngle) {
        bool toReturn = true;

        requiredDistance = GV2.GetDouble("MapMaker.MaxKFDistWiggleMult", 1.0, SILENT) * mdWiggleScaleDepthNormalized;
        requiredDistance *= kCurrent.dSceneDepthMean;
        vector<pair<double, KeyFrame*> > pClosest = ClosestKeyFrames(kCurrent, requiredDistance);

        if (pClosest.size() == 0) {
            // all keyframes are further than a min distance required
            return toReturn;
        } else {
            // does the new keyframe have a significantly different pose than all the existing keyframes?

            // Pose of the new keyframe, expressed as a pointing vector
            SO3<> currentWfromC = kCurrent.se3CfromW.get_rotation().inverse();
            Vector < 3 > kCurrrentBoresight = currentWfromC * makeVector(0, 0, 1);
            normalize(kCurrrentBoresight); //Should be normalized, but just in case

            currentDistance = DBL_MAX;
            currentAngle = DBL_MAX;
            requiredAngle = GV2.GetDouble("MapMaker.MinKFsVectorDiffInDeg", 25.0, SILENT);

            for (unsigned int i = 0; i < pClosest.size(); i++) {
                // Find and save current minimum distance
                if (pClosest[i].first < currentDistance)
                    currentDistance = pClosest[i].first;

                KeyFrame* kOld = pClosest[i].second;

                SO3<> oldWfromC = kOld->se3CfromW.get_rotation().inverse();
                Vector < 3 > kOldBoresight = oldWfromC * makeVector(0, 0, 1);
                normalize(kOldBoresight);
                // Compute the angle between vectors (in degrees)
                double vecDifference = acos(kCurrrentBoresight * kOldBoresight) * 180 / M_PI;

                // If any old keyframe has similar pose do not add the new keyframe
                if (vecDifference < requiredAngle) {
                    // Save current minimum angle and return false
                    currentAngle = vecDifference;
                    toReturn = false;
                }// This is not necessary, since it will be instantly invalidated with the addition of the new keyframe
                    // But it is useful for debugging, when new algorithm does not decide if we should drop a frame
                else if (currentAngle > vecDifference)
                    currentAngle = vecDifference;
            }

            // New keyframe has different pose
            return toReturn;
        }
    }
}
