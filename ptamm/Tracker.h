...
namespace PTAMM {
    ...
    class Tracker {
    public:
        Tracker(CVD::ImageRef irVideoSize, const ATANCamera &c, std::vector<Map*> &maps, Map *m, MapMaker &mm, CVD::Image<CVD::Rgb<CVD::byte> > *colorFrame, int cameraNumber);
        ...
        inline bool IsLost() {
            return (mnLostFrames > NUM_LOST_FRAMES);
        }
        ...
        void ForceRecovery() {
            if (mnLostFrames < NUM_LOST_FRAMES) mnLostFrames = NUM_LOST_FRAMES;
        }
        ...
        /******* <createdataset><secondcamera> *********/
        void writePosToFile(ofstream& f) {
            double conf = getConfidence();
            f << cameraNumber << "|" << conf << "|" << endl;
            f << GetCurrentPose();
            f << "$$";
        }
        /******* </secondcamera></createdataset> *********/
    protected:
        ...
        /*** <custom> ***/
        int cameraNumber;
        CVD::Image<CVD::Rgb<CVD::byte> > *colorFrame;
        SE3<> getIMUPosFromNavData(shm::nav_data data);
        void PublishTrackedPosAndMap(); // publish tracked pos to shdata
        void PublishMapPoints(); // publish tracked pos to shdata
        IplImage* mFirstCVFrame;
        void TrackForInitialMapUsingSURF(IplImage & imCVFrame); // This uses SURF instead of 'trail-tracking'
        void SURFTracking_Start(IplImage & imCVFrame);
        double getConfidence();
        int getNumFoundPoints();
        int getNumAttemptedPoints();
        /*** </custom> ***/
        ...
        void saveWallvizFrame(); // <custom><wallvizmap>
        void setSLAMScaleToMetricScale(); // <custom>
