...
class MapMaker : protected CVD::Thread
{
  public:
    ...
    /******* <custom><ptamv> *******/
    KeyFrame *mkSrc; // The new keyframe
    KeyFrame *mkTarget;
    KeyFrame *mkTargetBest;
    std::vector<std::pair<double, KeyFrame* > > ClosestKeyFrames(KeyFrame &k, double maxDistance);
    bool NeedNewKeyFrameExtMode(KeyFrame &kCurrent, double &currentDistance, double &requiredDistance, double &currentAngle, double &requiredAngle);
    KeyFrame* ClosestKeyFrame(KeyFrame &k, int &nClosest);
    KeyFrame* NewBestMatchKeyFrame(KeyFrame &k);
    void AddSomeMapPoints(int nLevel, KeyFrame &kSrc, KeyFrame &kTarget);
    int FindNrOfMatches(int nLevel, KeyFrame &kSrc, KeyFrame &kTarget);
    bool FindPointEpipolar(KeyFrame &kSrc, KeyFrame &kTarget, int nLevel, int nCandidate);
    /******* </ptamv></custom> *******/
